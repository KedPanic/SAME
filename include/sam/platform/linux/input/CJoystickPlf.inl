...
// Send input event.
INLINE void CJoystick::SendEvent(SInputEvent &p_oEvent)
{
	// send button event.
	p_oEvent.m_nModifiers = 0;
	p_oEvent.m_sKeyName = g_aKeyName[p_oEvent.m_eKey];

	// send input event.
	g_Env->pInputManager->FireInputEvent(p_oEvent);
}
